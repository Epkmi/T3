...
void clearLifeGlyph(int lives) {
    ...
    int startX = LCDWIDTH - 1 - lives;

    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            nokia_lcd_set_pixel(startX, i, 0);
        }
    }
}
...
void generateRandomObject(Object *obj, int *objectsInCenter) {
...
int main(void) {
    ...
    while (1) {
        ...
        if (rand() % 100 < 20) { // Ajuste o valor para controlar a frequência de geração
            generateRandomObject(&movingObject, &objectsInCenter);
            targetX = (LCDWIDTH - OBJECT_WIDTH) / 2;
            targetY = (LCDHEIGHT - OBJECT_HEIGHT) / 2;
        }

        // Mova o objeto em direção ao centro
        moveObject(&movingObject, targetX, targetY, &lives);

        // Exiba o objeto em sua nova posição
        displayAnotherGlyph(movingObject.x, movingObject.y);
